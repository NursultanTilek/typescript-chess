 ...
 export abstract class LongRangePiece extends Piece{
    ...
    override isSquareAvailableForAttack(coordination: Coordination,boardCondition:Map<CoordinationId, PieceType>): boolean {
        ...
        if (this.coordination.file == coordination.file) {
            coordinationBetween = BoardUtils.getVerticalCoordinationBetween(this.coordination, coordination);
        } else if (this.coordination.rank == coordination.rank) {
            coordinationBetween = BoardUtils.getHorizontalCoordinationBetween(this.coordination, coordination);
        } else {
            coordinationBetween = BoardUtils.getDiagonalCoordinationBetween(this.coordination, coordination);
        }
