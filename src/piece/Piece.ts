...
export abstract class Piece  {
    ...
    const boardCondition=usePieces.getState().pieces
      ...
      for(const shift of this.getPieceMoves()){
              if(this.coordination.canShift(shift)){
                  const coordination=new Coordination(this.coordination.file,this.coordination.rank)
                  const newCoordination=coordination.shift(shift)
                  if(this.isSquareAvailableForMove(newCoordination) && this.isSquareAvailableForAttack(newCoordination,boardCondition)){
  ...
  protected isSquareAvailableForMove(coordination:Coordination){
    const boardCondition=usePieces.getState().pieces
    return !this.isSamePieceColor(boardCondition,coordination)||this.isSquareEmpty(boardCondition,coordination)

  }

  protected isSquareEmpty(boardCondition:Map<CoordinationId, PieceType>,coordination:Coordination){
    return !boardCondition.get(coordination.id) 
  }

  private isSamePieceColor(boardCondition:Map<CoordinationId, PieceType>,coordination:Coordination){
    return boardCondition.get(coordination.id)?.color===this.color
  }

  protected getPiecesByColor(color: Color,board:Map<CoordinationId,PieceType>): Piece[] {
      const result: Piece[] = [];
...
//TODO 2) DO NOT FORGET ABOUT DISCOVERY ATTACK TO THE OWN KING , AFTER YOU DID THIS , SOLVE THIS LIKE i DID ON KING CLASS LIKE THIS FUNCTION isMoveSafe()
    ...
    protected isSquareAttackedByEnemy(coordination: Coordination, color: Color, boardCondition:Map<CoordinationId,PieceType>): boolean {
  ...
  public getAttackedSquares(boardCondition:Map<CoordinationId, PieceType>): Set<CoordinationId> {
      ...
      for (const pieceAttack of pieceAttacks) {
          if (this.coordination.canShift(pieceAttack)) {
              const shiftedCoordinates: Coordination = this.coordination.shift(pieceAttack);    
              if (this.isSquareAvailableForAttack(shiftedCoordinates,boardCondition)) {
  ...
  protected isSquareAvailableForAttack(coordination:Coordination,boarCondtion:Map<CoordinationId, PieceType>):boolean{
